...
esp_err_t handle_oil_detection(httpd_req_t *req) {
    ...
    for (int i = 0; i < (fb->len / 2); i++) {
        if (isGoodOil(image[i])) countGood++;
        if (isBadOil(image[i])) countBad++;
    }

    goodOilCount += countGood;  // ✅ Keep track of good oil
    badOilCount += countBad;    // ✅ Keep track of bad oil

    esp_camera_fb_return(fb);

    char response[100];
    sprintf(response, "{\"good_oil\": %d, \"bad_oil\": %d}", goodOilCount, badOilCount);
    httpd_resp_set_type(req, "application/json");
    return httpd_resp_send(req, response, strlen(response));
}

// ✅ Function to Enable/Disable Oil Detection (ONLY from /secret page)
esp_err_t set_oil_mode(httpd_req_t *req) {
    ...
    if (httpd_query_key_value(req->uri, "oil", param, sizeof(param)) == ESP_OK) {
        if (strcmp(param, "good") == 0) {
            detectGoodOil = true;
            detectBadOil = false;
            goodOilCount++;  // ✅ Increase count
            Serial.println("Good Oil detected!");  
        } else if (strcmp(param, "bad") == 0) {
            detectGoodOil = false;
            detectBadOil = true;
            badOilCount++;  // ✅ Increase count
            Serial.println("Bad Oil detected!");  
        } else {
            detectGoodOil = false;
            detectBadOil = false;
            Serial.println("Oil detection disabled.");
        }
